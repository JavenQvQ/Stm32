...
uint16_t fps = 0; //帧率
uint8_t update_flag = 0; //全局更新标志
extern uint16_t time_out;
extern Alarm alarm;
extern Calender time;
...
Icon main_icon_array[MAIN_PAGE_NUM] = 
{
    ...
    [3] = {0x00,0x00,0xC0,0xE0,0xF0,0x38,0x1C,0x1C,0x0E,0x06,0x07,0x07,0x07,0x03,0x03,0x03,
...
void MainPage_CallBack(uint8_t self_page_id,Option* select_item)
{
    switch (select_item->order)
    {
        ...
        case 2: OLED_UIJumpToPage(self_page_id,&alarm1_page); break;
        case 3: OLED_UIJumpToPage(self_page_id,&about_page); break;
        default: break;
    }
}
...
void Ring_CallBack(uint8_t self_page_id,Option* select_item)
{
    ...
    {
        OLED_WinDrawStr(&w_all, 60, 8, 16, "Time OUT");
    ...
    if(!FLAG_IS_SET(update_flag, ALARM1_RING_MSK))
    {
        MyRTC_ReadTime(&time);
            ...
            time_out=alarm.sec-time.sec;
            else if (alarm.min-1==time.min&&alarm.sec<=time.sec)
            time_out=alarm.sec+60-time.sec;
        sprintf(Countdown, "%d", time_out);
        OLED_WinDrawStr(&w_all, 60, 8, 16, "Countdown");
        OLED_WinDrawStr(&w_all, 80, 30, 24,Countdown);        
    }
...
void TestUI_Init(void)
{
    ...
    OLED_DigitalPageInit(&calendar_page, calendar_page_id, calendar_option_array, CALENDAR_LABEL_NUM, week_str, '-', 50, 100, CalendarPage_CallBack);
