...
static void I2C_Mode_Configu(void)
{
  ...
  I2C_InitStructure.I2C_AcknowledgedAddress = I2C_AcknowledgedAddress_7bit;
	...
	/* 通信速率 */
  I2C_InitStructure.I2C_ClockSpeed = I2C_Speed;//100K
...
{
	 u16 size = strlen((const char *)str) + 2;
...
void SetBase(u8 *str)
{
	 ...
	 I2C_Writes_Bytes(DataPack.Text, strlen((const char *)DataPack.Text));
}
...
void SetStyle(Style_Type style)//合成风格设置 [f?]
{
  TextCtrl('f', style);
...
void SetLanguage(Language_Type language)//合成语种设置 [g?]
{
	  TextCtrl('g', language);
		while(GetChipStatus() != ChipStatus_Idle)
		{
...
void SetArticulation(Articulation_Type articulation)//设置单词的发音方式 [h?]
{
...
void SetSpell(Spell_Type spell)//设置对汉语拼音的识别 [i?]
{
  TextCtrl('i', spell);
	while(GetChipStatus() != ChipStatus_Idle)
	{
...
void SetReader(Reader_Type reader)//设置发音人 [m?]
{
  TextCtrl('m', reader);
	while(GetChipStatus() != ChipStatus_Idle)
	{
...
void SetNumberHandle(NumberHandle_Type numberHandle)//设置数字处理策略 [n?]
{
...
void SetZeroPronunciation(ZeroPronunciation_Type zeroPronunciation)//设置数字“0”的读音 [o?]
{
...
void SetNamePronunciation(NamePronunciation_Type namePronunciation)//设置姓名读音 策略 [r?]
{
...
void SetRhythm(Rhythm_Type rhythm)//是否使用韵律 标记“*”和“#” [z?]
{
  TextCtrl('z', rhythm);
	while(GetChipStatus() != ChipStatus_Idle)
	{
